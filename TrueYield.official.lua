if IY_LOADED and not _G.IY_DEBUG == true then
	loadstring(game:HttpGet('https://raw.githubusercontent.com/ffedex/ffedex.github.io/refs/heads/main/TrueYield.AlreadyExecuted'))()
-- .AlreadyExecuted file because why not?
	return
end
...
if not game:IsLoaded() then
	...
	notLoaded.Text = 'True Client is waiting for the game to load'
...
if os.date("*t", os.time()).month == 4 then
	...
	if string.find(currentVersion, '6.1') then
		Title.Text = "True Yield v" .. currentVersion
	else
		Title.Text = "True Yield v"  .. currentVersion
	end
end
...
Logo.Image = "rbxassetid://137255448457995"
...
Credits.Text = "fede, rbx thatonestandexx"
...
reference = (function()
	...
	inviteButton.MouseButton1Click:Connect(function()
		...
		inviteButton.Text = "Copy Discord Invite Link (nolink)"
	end)
	dragGUI(main)
	main.Parent = PARENT
...
function saves()
	if writefileExploit() and jsonAttempts < 50 then
		if pcall(function() readfile("TrueYield.tiy") end) then
			if readfile("TrueYield.tiy") ~= nil then
				local success, response = pcall(function()
					local json = HttpService:JSONDecode(readfile("TrueYield.tiy"))
				...
				if not success then
					...
					writefileCooldown("TrueYield.tiy", defaults)
					task.wait()
					saves()
				end
			else
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("TrueYield.tiy", HttpService:JSONEncode(update))
	end
end
...
CMDs[#CMDs + 1] = {NAME = 'sunc / sunctest', DESC = 'Tests sUNC'}
...
function addPlugin(name)
	if name:lower() == 'plugin file name' or name:lower() == 'TrueYield.tiy' or name == 'TrueYield' then
		notify('Plugin Error','Please enter a valid plugin')
	else
		local file
		local fileName
		if name:sub(-3) == '.tiy' then
			pcall(function() file = readfile(name) end)
			fileName = name
		else
			pcall(function() file = readfile(name..'.tiy') end)
			fileName = name..'.tiy'
		end
		if file then
...
addcmd('ssppy',{},function(args, speaker)
	loadstring(game:HttpGet("https://raw.githubusercontent.com/ffedex/ffedex.github.io/refs/heads/main/SimpleSpy.TrueYield"))("SimpleSpy")
end)
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then
		notify("True Client has loaded. Made by Fede! ffedex!")
		if game.PlaceId == 574746640 then
			task.wait(3)
